	...
	public DomainParticipant getObject() throws Exception {
		if (null == instance) {
			...
			if (!discoveryPeers.isEmpty()) {
				...
				// is cleared and the RTI discovery process will not listen for discovery
				// messages via multicast.
				//
				// If NDDS_DISCOVERY_PEERS contains one or more multicast addresses, the
				// addresses will be stored in
				// com.rti.dds.infrastructure.DiscoveryQosPolicy.multicast_receive_addresses,
				// starting at element 0. They will be stored in the order they appear
				// NDDS_DISCOVERY_PEERS.
				//
				// Note: Currently, RTI Connext will only listen for discovery traffic on the
				// first multicast address (element 0) in
				// com.rti.dds.infrastructure.DiscoveryQosPolicy.multicast_receive_addresses.
			...
			if (dpQos.discovery.multicast_receive_addresses.size() != 0)
				log.warn("Discovery 'multicast_receive_addresses' size:"
						+ dpQos.discovery.multicast_receive_addresses.size() + " first entry:"
						+ dpQos.discovery.multicast_receive_addresses.get(0).toString());

			instance = com.rti.dds.domain.DomainParticipantFactory.get_instance().create_participant(domain, dpQos,
					null, StatusKind.STATUS_MASK_NONE);

			// Initialize this builtin reader
			instance.get_builtin_subscriber()
