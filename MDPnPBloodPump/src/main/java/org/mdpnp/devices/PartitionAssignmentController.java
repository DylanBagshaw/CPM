...
public class PartitionAssignmentController implements MDSHandler.Objective.MDSListener {
	...
	public PartitionAssignmentController(DeviceIdentity d, DomainParticipant dp, EventLoop e, Publisher p,
			Subscriber s) {
		deviceIdentity = d;
		publisher = p;
		subscriber = s;
	...
	public static class PersistentPartitionAssignment extends PartitionAssignmentController {
		...
		public PersistentPartitionAssignment(ScheduledExecutorService executor, String f, DeviceIdentity d,
				DomainParticipant dp, EventLoop e, Publisher p, Subscriber s) {
			super(d, dp, e, p, s);
		...
		public void start() {
			...
			heartbeatTask = executor.scheduleAtFixedRate(() -> checkForPartitionFile(), HEARTBEAT_INTERVAL,
