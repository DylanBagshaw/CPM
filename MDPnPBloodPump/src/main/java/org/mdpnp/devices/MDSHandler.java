 ...
 * @author mfeinberg
 *
 *         Container for both sides. Each side can be instaciated separately if
 *         needs to
 */
public class MDSHandler {
	...
	public MDSHandler(EventLoop eventLoop, DomainParticipant dp) {
		...
		mdsSubscriber = dp.create_subscriber(DomainParticipant.SUBSCRIBER_QOS_DEFAULT, null,
				StatusKind.STATUS_MASK_NONE);
		setCommunicationChannel(mdsPublisher, mdsSubscriber);

		wildcardPublisher = dp.create_publisher(DomainParticipant.PUBLISHER_QOS_DEFAULT, null,
				StatusKind.STATUS_MASK_NONE);
		wildcardSubscriber = dp.create_subscriber(DomainParticipant.SUBSCRIBER_QOS_DEFAULT, null,
				StatusKind.STATUS_MASK_NONE);
	...
	static void shutdown(EventLoop eventLoop, Subscriber subscriber, Publisher publisher, DataReaderImpl reader,
			DataWriterImpl writer, Topic topic, ReadCondition readCondition) {
	...
	public static class Connectivity {
		...
		public void start() {
			...
			eventLoop.addHandler(mdsoReadCondition, new EventLoop.ConditionHandler() {
				...
				public void conditionChanged(Condition condition) {
					try {
						mdsoReader.read_w_condition(data_seq, info_seq, ResourceLimitsQosPolicy.LENGTH_UNLIMITED,
								mdsoReadCondition);
		...
		public void shutdown() {
			...
			MDSHandler.shutdown(eventLoop, subscriber, publisher, mdsoReader, mdsoWriter, msdoConnectivityTopic,
					mdsoReadCondition);
		}
		...
		public Connectivity(EventLoop eventLoop, Publisher publisher, Subscriber subscriber) {
			...
			msdoConnectivityTopic = TopicUtil.findOrCreateTopic(participant, ice.MDSConnectivityTopic.VALUE,
					ice.MDSConnectivityTypeSupport.class);

			mdsoReader = (ice.MDSConnectivityDataReader) subscriber.create_datareader_with_profile(
					msdoConnectivityTopic, QosProfiles.ice_library, QosProfiles.state, null,
					StatusKind.STATUS_MASK_NONE);
			...
			mdsoWriter = (ice.MDSConnectivityDataWriter) publisher.create_datawriter_with_profile(msdoConnectivityTopic,
	...
	public static class Objective {
		...
		public Objective(EventLoop eventLoop, Publisher publisher, Subscriber subscriber) {
			...
			ice.MDSConnectivityObjectiveTypeSupport.register_type(participant,
					ice.MDSConnectivityObjectiveTypeSupport.get_type_name());

			msdoConnectivityTopic = (Topic) TopicUtil.findOrCreateTopic(participant,
					ice.MDSConnectivityObjectiveTopic.VALUE, ice.MDSConnectivityObjectiveTypeSupport.class);

			mdsoReader = (ice.MDSConnectivityObjectiveDataReader) subscriber.create_datareader_with_profile(
					msdoConnectivityTopic, QosProfiles.ice_library, QosProfiles.state, null,
					StatusKind.STATUS_MASK_NONE);
			...
			mdsoWriter = (ice.MDSConnectivityObjectiveDataWriter) publisher.create_datawriter_with_profile(
					msdoConnectivityTopic, QosProfiles.ice_library, QosProfiles.state,
					new LogEntityStatus(log, "MDSConnectivity"), StatusKind.STATUS_MASK_ALL);
	...
	public static class Patient {
		...
		public void start() {
			...
			eventLoop.addHandler(patientReadCondition, new EventLoop.ConditionHandler() {
				...
				public void conditionChanged(Condition condition) {
					try {
						patientReader.read_w_condition(data_seq, info_seq, ResourceLimitsQosPolicy.LENGTH_UNLIMITED,
								patientReadCondition);
						for (int i = 0; i < info_seq.size(); i++) {
							SampleInfo si = (SampleInfo) info_seq.get(i);
							if (si.valid_data) {
		...
		public void shutdown() {
			...
			MDSHandler.shutdown(eventLoop, subscriber, publisher, patientReader, patientWriter,
		...
		public Patient(EventLoop eventLoop, Publisher publisher, Subscriber subscriber) {
			...
			patientConnectivityTopic = TopicUtil.findOrCreateTopic(participant, ice.PatientTopic.VALUE,
					ice.PatientTypeSupport.class);

			patientReader = (ice.PatientDataReader) subscriber.create_datareader_with_profile(patientConnectivityTopic,
					QosProfiles.ice_library, QosProfiles.state, null, StatusKind.STATUS_MASK_NONE);

			patientReadCondition = patientReader.create_readcondition(SampleStateKind.NOT_READ_SAMPLE_STATE,
					ViewStateKind.ANY_VIEW_STATE, InstanceStateKind.ANY_INSTANCE_STATE);

			patientWriter = (ice.PatientDataWriter) publisher.create_datawriter_with_profile(patientConnectivityTopic,
					QosProfiles.ice_library, QosProfiles.state, null, StatusKind.STATUS_MASK_NONE);
