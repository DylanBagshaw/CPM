...
/**
 * A sequence of GlobalAlarmLimitObjective instances.
 */
public final class GlobalAlarmLimitObjectiveSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    public GlobalAlarmLimitObjectiveSeq() {
        super(GlobalAlarmLimitObjective.class);
    }


    public GlobalAlarmLimitObjectiveSeq(int initialMaximum) {
        super(GlobalAlarmLimitObjective.class, initialMaximum);
    }


    public GlobalAlarmLimitObjectiveSeq(Collection elements) {
        super(GlobalAlarmLimitObjective.class, elements);
    }
    ...
    public Object copy_from(Object src) {
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    set(i, GlobalAlarmLimitObjective.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }
        
        // copy 'new' GlobalAlarmLimitObjective objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(GlobalAlarmLimitObjective.create());
