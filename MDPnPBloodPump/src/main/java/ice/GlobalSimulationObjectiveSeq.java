...
/**
 * A sequence of GlobalSimulationObjective instances.
 */
public final class GlobalSimulationObjectiveSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    public GlobalSimulationObjectiveSeq() {
        super(GlobalSimulationObjective.class);
    }


    public GlobalSimulationObjectiveSeq(int initialMaximum) {
        super(GlobalSimulationObjective.class, initialMaximum);
    }


    public GlobalSimulationObjectiveSeq(Collection elements) {
        super(GlobalSimulationObjective.class, elements);
    }
    ...
    public Object copy_from(Object src) {
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    set(i, GlobalSimulationObjective.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }
        
        // copy 'new' GlobalSimulationObjective objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(GlobalSimulationObjective.create());
