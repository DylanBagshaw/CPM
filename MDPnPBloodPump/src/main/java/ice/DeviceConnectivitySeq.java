...
/**
 * A sequence of DeviceConnectivity instances.
 */
public final class DeviceConnectivitySeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    public DeviceConnectivitySeq() {
        super(DeviceConnectivity.class);
    }


    public DeviceConnectivitySeq(int initialMaximum) {
        super(DeviceConnectivity.class, initialMaximum);
    }


    public DeviceConnectivitySeq(Collection elements) {
        super(DeviceConnectivity.class, elements);
    }
    ...
    public Object copy_from(Object src) {
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    set(i, DeviceConnectivity.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }
        
        // copy 'new' DeviceConnectivity objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(DeviceConnectivity.create());
