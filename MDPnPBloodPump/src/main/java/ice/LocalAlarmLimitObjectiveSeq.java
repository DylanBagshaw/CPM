...
/**
 * A sequence of LocalAlarmLimitObjective instances.
 */
public final class LocalAlarmLimitObjectiveSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    public LocalAlarmLimitObjectiveSeq() {
        super(LocalAlarmLimitObjective.class);
    }


    public LocalAlarmLimitObjectiveSeq(int initialMaximum) {
        super(LocalAlarmLimitObjective.class, initialMaximum);
    }


    public LocalAlarmLimitObjectiveSeq(Collection elements) {
        super(LocalAlarmLimitObjective.class, elements);
    }
    ...
    public Object copy_from(Object src) {
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    set(i, LocalAlarmLimitObjective.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }
        
        // copy 'new' LocalAlarmLimitObjective objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(LocalAlarmLimitObjective.create());
