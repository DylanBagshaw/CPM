...
/**
 * A collection of useful methods for dealing with objects of type Image.
 */
public class ImageTypeSupport extends TypeSupportImpl {
	// -----------------------------------------------------------------------
	// Private Fields
	// -----------------------------------------------------------------------

	private static final String TYPE_NAME = "ice::Image";

	private static final char[] PLUGIN_VERSION = { 2, 0, 0, 0 };

	public static final int LAST_MEMBER_ID = 1;

	private static final ImageTypeSupport _singleton = new ImageTypeSupport();
	...
	public static void register_type(DomainParticipant participant, String type_name) {
	...
	/*
	 * The methods in this section are for use by RTI Connext itself and by the code
	 * generated by rtiddsgen for other types. They should be used directly or
	 * modified only by advanced users and are subject to change in future versions
	 * of RTI Connext.
	 */
	public static ImageTypeSupport get_instance() {
		return _singleton;
	}

	public static ImageTypeSupport getInstance() {
		return get_instance();
	}
	 ...
	 * class. This method will perform a deep copy of <code>source</code> into
	 ...
	 *                If <code>destination</code> or <code>source</code> is null.
	 * @exception ClassCastException
	 *                If either <code>destination</code> or <code>this</code> is not
	 *                a <code>Image</code> type.
	 */
	public Object copy_data(Object destination, Object source) {
		Image typedDst = (Image) destination;
		Image typedSrc = (Image) source;
	...
	public long get_serialized_sample_max_size(Object endpoint_data, boolean include_encapsulation,
		...
		currentAlignment += ice.ImageDataTypeSupport.get_instance().get_serialized_sample_max_size(endpoint_data, false,
				encapsulation_id, currentAlignment);
	...
	public long get_serialized_sample_min_size(Object endpoint_data, boolean include_encapsulation,
		...
		currentAlignment += ice.ImageDataTypeSupport.get_instance().get_serialized_sample_min_size(endpoint_data, false,
				encapsulation_id, currentAlignment);
	...
	public long get_serialized_sample_size(Object endpoint_data, boolean include_encapsulation, short encapsulation_id,
		...
		currentAlignment += CdrPrimitiveType.getStringSerializedSize(currentAlignment, typedSrc.content_type);

		currentAlignment += ice.ImageDataTypeSupport.get_instance().get_serialized_sample_size(endpoint_data, false,
				encapsulation_id, currentAlignment, typedSrc.image);
	...
	public void serialize(Object endpoint_data, Object src, CdrOutputStream dst, boolean serialize_encapsulation,
		...
		if (serialize_sample) {
			...
			dst.writeString(typedSrc.content_type, (64));

			ice.ImageDataTypeSupport.get_instance().serialize(endpoint_data, typedSrc.image, dst, false,
					encapsulation_id, true, endpoint_plugin_qos);
	...
	public Object deserialize_sample(Object endpoint_data, Object dst, CdrInputStream src,
		...
		if (deserialize_sample) {
			Image typedDst = (Image) dst;

			typedDst.clear();
			try {

				typedDst.content_type = src.readString((64));

				typedDst.image = (ice.ImageData) ice.ImageDataTypeSupport.get_instance()
						.deserialize_sample(endpoint_data, typedDst.image, src, false, true, endpoint_plugin_qos);
	...
	public void skip(Object endpoint_data, CdrInputStream src, boolean skip_encapsulation, boolean skip_sample,
		...
		if (skip_sample) {
			...
			ice.ImageDataTypeSupport.get_instance().skip(endpoint_data, src, false, true, endpoint_plugin_qos);
	...
	public Object on_participant_attached(Object registration_data, TypeSupportParticipantInfo participant_info,
			boolean top_level_registration, Object container_plugin_context, TypeCode type_code) {
		return super.on_participant_attached(registration_data, participant_info, top_level_registration,
	...
	public Object on_endpoint_attached(Object participantData, TypeSupportEndpointInfo endpoint_info,
			boolean top_level_registration, Object container_plugin_context) {
		return super.on_endpoint_attached(participantData, endpoint_info, top_level_registration,
	...
	protected DataWriter create_datawriter(long native_writer, DataWriterListener listener, int mask) {

		return null;

	}

	protected DataReader create_datareader(long native_reader, DataReaderListener listener, int mask) {
	...
	protected ImageTypeSupport() {

		/*
		 * If the user data type supports keys, then the second argument to the
		 * constructor below should be true. Otherwise it should be false.
		 */

		super(TYPE_NAME, false, ImageTypeCode.VALUE, Image.class, TypeSupportType.TST_STRUCT, PLUGIN_VERSION);

	}

	protected ImageTypeSupport(boolean enableKeySupport) {

		super(TYPE_NAME, enableKeySupport, ImageTypeCode.VALUE, Image.class, TypeSupportType.TST_STRUCT,
				PLUGIN_VERSION);
	}
}
