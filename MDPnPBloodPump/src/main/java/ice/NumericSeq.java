...
/**
 * A sequence of Numeric instances.
 */
public final class NumericSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    public NumericSeq(int initialMaximum) {
        super(Numeric.class, initialMaximum);
    }


    public NumericSeq(Collection elements) {
        super(Numeric.class, elements);
    }
    ...
    public Object copy_from(Object src) {
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    set(i, Numeric.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }
        
        // copy 'new' Numeric objects (beyond the original size of this object)
