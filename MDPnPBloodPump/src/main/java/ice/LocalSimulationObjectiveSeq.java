...
/**
 * A sequence of LocalSimulationObjective instances.
 */
public final class LocalSimulationObjectiveSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    public LocalSimulationObjectiveSeq() {
        super(LocalSimulationObjective.class);
    }


    public LocalSimulationObjectiveSeq(int initialMaximum) {
        super(LocalSimulationObjective.class, initialMaximum);
    }


    public LocalSimulationObjectiveSeq(Collection elements) {
        super(LocalSimulationObjective.class, elements);
    }
    ...
    public Object copy_from(Object src) {
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    set(i, LocalSimulationObjective.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }
        
        // copy 'new' LocalSimulationObjective objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(LocalSimulationObjective.create());
