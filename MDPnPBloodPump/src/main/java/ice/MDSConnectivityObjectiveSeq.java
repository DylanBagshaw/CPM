...
/**
 * A sequence of MDSConnectivityObjective instances.
 */
public final class MDSConnectivityObjectiveSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    public MDSConnectivityObjectiveSeq() {
        super(MDSConnectivityObjective.class);
    }


    public MDSConnectivityObjectiveSeq(int initialMaximum) {
        super(MDSConnectivityObjective.class, initialMaximum);
    }


    public MDSConnectivityObjectiveSeq(Collection elements) {
        super(MDSConnectivityObjective.class, elements);
    }
    ...
    public Object copy_from(Object src) {
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    set(i, MDSConnectivityObjective.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }
        
        // copy 'new' MDSConnectivityObjective objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(MDSConnectivityObjective.create());
