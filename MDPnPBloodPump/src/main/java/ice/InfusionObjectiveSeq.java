...
/**
 * A sequence of InfusionObjective instances.
 */
public final class InfusionObjectiveSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    public InfusionObjectiveSeq() {
        super(InfusionObjective.class);
    }


    public InfusionObjectiveSeq(int initialMaximum) {
        super(InfusionObjective.class, initialMaximum);
    }


    public InfusionObjectiveSeq(Collection elements) {
        super(InfusionObjective.class, elements);
    }
    ...
    public Object copy_from(Object src) {
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    set(i, InfusionObjective.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }
        
        // copy 'new' InfusionObjective objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(InfusionObjective.create());
