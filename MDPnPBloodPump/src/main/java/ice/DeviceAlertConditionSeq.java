...
/**
 * A sequence of DeviceAlertCondition instances.
 */
public final class DeviceAlertConditionSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    public DeviceAlertConditionSeq() {
        super(DeviceAlertCondition.class);
    }


    public DeviceAlertConditionSeq(int initialMaximum) {
        super(DeviceAlertCondition.class, initialMaximum);
    }


    public DeviceAlertConditionSeq(Collection elements) {
        super(DeviceAlertCondition.class, elements);
    }
    ...
    public Object copy_from(Object src) {
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    set(i, DeviceAlertCondition.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }
        
        // copy 'new' DeviceAlertCondition objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(DeviceAlertCondition.create());
