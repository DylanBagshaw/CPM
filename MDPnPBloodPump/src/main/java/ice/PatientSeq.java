...
/**
 * A sequence of Patient instances.
 */
public final class PatientSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    public PatientSeq(int initialMaximum) {
        super(Patient.class, initialMaximum);
    }


    public PatientSeq(Collection elements) {
        super(Patient.class, elements);
    }
    ...
    public Object copy_from(Object src) {
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    set(i, Patient.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }
        
        // copy 'new' Patient objects (beyond the original size of this object)
