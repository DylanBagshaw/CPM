...
/**
 * A sequence of TimeSync instances.
 */
public final class TimeSyncSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    public TimeSyncSeq() {
        super(TimeSync.class);
    }


    public TimeSyncSeq(int initialMaximum) {
        super(TimeSync.class, initialMaximum);
    }


    public TimeSyncSeq(Collection elements) {
        super(TimeSync.class, elements);
    }
    ...
    public Object copy_from(Object src) {
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    set(i, TimeSync.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }
        
        // copy 'new' TimeSync objects (beyond the original size of this object)
